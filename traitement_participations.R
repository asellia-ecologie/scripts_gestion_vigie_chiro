...
for (parti in participations){ # Pour chaque participation :
    ...
    participation$Date_Heure <- ymd_hms(
                 paste(
                      str_split_i(participation$nom.du.fichier,
                                  "_", -3),
                      str_split_i(participation$nom.du.fichier,
                                  "_", -2)))
    ...
    # on va récupérer l’étude de la participation dans la bdd
       ...
       etude <- dbGetQuery(connec,
                      paste0("select etude from bd_sons.bdd_placettes
    ...
    ordre_colonnes <- c("nom_du_fichier", "Date_Nuit", "Type", "temps_debut",
...
# on écrut le fichier dans un xlsx par étude (ouverture et ajout s’il existe, création sinon)
    if (file.exists(paste0(dos_compils, "/compilation_", unique(year(date_nuit)),
                       "_", etude, ".xlsx"))) {
        a <- read_excel(paste0(dos_compils, "/compilation_", unique(year(date_nuit)),
                       "_", etude, ".xlsx"))
        p <- rbind(a, participation)
        write_xlsx(p, paste0(dos_compils, "/compilation_",
                             unique(year(date_nuit)), "_", etude, ".xlsx"))
    } else {
        write_xlsx(participation, paste0(dos_compils, "/compilation_",
