...
use_python("/home/asellia_pc_chiro/miniconda3/envs/Gestion_Sons/bin/python")
setwd("/home/asellia_pc_chiro/Bureau/GESTION_SONS/")
```
...
import datetime
from id_bdd import USER, PASSWORD, HOST, PORT, DB_NAME, HOM # imports param conn.
...
os.chdir(HOME)
# os.chdir('/media/asellia_pc_chiro/T_01/02_SONS_A_TRAITER')
#os.dir ('')
...
### Vérification des nuits/boitier avant codage vigie-chiro
```{python}
...
def get_placette_count(cursor, boitier, date):
    query = '''SELECT count(distinct(id)) FROM bd_sons.bdd_placettes_2023 WHERE num_sm2 = %s
...
def get_ligne(cursor, boitier, date):
    query = """select split_part(obs1, ' ',2) as FirstName, split_part(obs1, ' ',1) as FamilyName,
        'asellia.ecologie@gmail.com' as Email, 'Asellia Écologie' as Affiliation,
        'France' as Country, nom_point as Site, st_x(geom) as X, st_y(geom) as Y,
        'Pass'||passage as Participation, to_char("date",'DD/MM/YYYY')  as StartDate, 
        to_char("date" + interval '1 day' * nombre_de_nuits, 'DD/MM/YYYY') as EndDate, 
        1 as TypeStudy, hauteur_micro as MicHeight,
        case
...
with contextlib.ExitStack() as ctx:
    ...
    for _, row in nuits_boitiers.iterrows():
        rang = dict(row)
        ligne = get_ligne(cursor, rang['boitier'], rang['date'])
        print(ligne)
...
def updateVigie(cursor, site, carre):
    sqlUpdateCarre = f''' update bd_sons.bdd_placettes_2023
    set vigie_chiro = %s
    where nom_point = %s
...
def updatePoint(cursor, site, point):
    sqlUpdatePoint = f''' update bd_sons.bdd_placettes_2023
    set code_point = %s
    where nom_point = %s;
    '''
...
r.beep(sound = 1)
```
Puis mise à jour des Participations :
## Update participation

```{python}

# Mettre ici le nom du csv a_coder_..._created.csv
participations = pd.read_csv(f'{a_nommer}/a_coder20231129_122413_ok_created.csv', sep = ";")

def updateVigieId(cursor, id_site, site, date):
    date_split = date.split(os.sep)
    date = date_split[2] + '-' + date_split[1] + '-' + date_split[0]
    print(date)
    print(id_site)
    print(site)
    sqlUpdateVigieId = f''' update bd_sons.bdd_placettes_2023
    set site_vigie = %s
    where nom_point = %s
    and "date" = %s;
    '''
...
def updateParticipationId(cursor, participation, site, date):
    date_split = date.split(os.sep)
    date = date_split[2] + '-' + date_split[1] + '-' + date_split[0]
    sqlUpdateParticipation = f''' update bd_sons.bdd_placettes_2023
...
```{python}
def getSiteParti(cursor, placette, date):
